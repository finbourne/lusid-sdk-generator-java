...
public class RefreshingTokenProviderTest {
    ...
    public void get_OnNoCurrentToken_ShouldCallLusidProviderForFullAuthentication() throws FinbourneTokenException {
        ...
        // ensure retrieved token using api call and did so via full authentication
        // route
    ...
    public void get_OnExpiredCurrentToken_ShouldCallLusidForNewRefreshedToken() throws FinbourneTokenException {
        ...
        // mock retrieving a refreshed token when we attempt to call api with a valid
        // refresh parameter
    ...
    @Test
    public void get_OnExpiredRefreshToken_ShouldCallLusidProviderForFullReauthentication()
            throws FinbourneTokenException {
        // Test the less often case where the REFRESH token itself has expired and
        // attempting to refresh is not
        ...
        doThrow(new FinbourneTokenException("Refresh token has expired")).when(httpFinbourneTokenProvider)
                .get(Optional.of("refresh_01"));
        ...
        // fetch token again. as both the access and refresh token have expired expect
        // an exception and a full
        // reauthentication attempt
        FinbourneToken nextToken = tokenProvider.get();

        assertThat(nextToken, not(equalTo(currentToken)));
        assertThat(nextToken, equalTo(newTokenWithNewRefresh));
        // ensure make initial full auth call, a refresh attempt and another full auth
        // call
    ...
    @Test
    public void get_OnFailedRefreshAndFailedReAuthAttempt_ShouldReThrowFinbourneTokenException()
            throws FinbourneTokenException {
        FinbourneToken expiredToken = new FinbourneToken("access_01", "refresh_01", LocalDateTime.MIN);
        doReturn(expiredToken).when(httpFinbourneTokenProvider).get(Optional.empty());
        ...
        doThrow(new FinbourneTokenException("Reauthentication attempt has failed")).when(httpFinbourneTokenProvider)
                .get(Optional.empty());
    ...
    @Test
    public void get_OnFailedRefreshAuthAttempt_ShouldReThrowFinbourneTokenExceptionWithCause()
            throws FinbourneTokenException {
        FinbourneToken expiredToken = new FinbourneToken("access_01", "refresh_01", LocalDateTime.MIN);
        doReturn(expiredToken).when(httpFinbourneTokenProvider).get(Optional.empty());
        ...
        doThrow(new FinbourneTokenException("Refresh token has expired", cause)).when(httpFinbourneTokenProvider)
                .get(Optional.of(currentToken.getRefreshToken()));
